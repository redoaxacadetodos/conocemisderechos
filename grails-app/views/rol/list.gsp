...
<html>
	<head>
		...
		<g:datatablehelperjs ctrlid="table"
			context="${request.getContextPath()}" controller="rol"
		...
		<script type="text/javascript">
			function mostrar(source, type, val){
				return "<a href='"+ CONTEXT_ROOT+"/rol/show/"+ source[0] + "'> "+ source[1] +"</a> ";
			}
		</script>
	</head>
	<body>
		...
		<div id="list-rol" class="content scaffold-list" role="main">
			<h1><g:message code="mx.gob.redoaxaca.rol.lista.label" default="Listado" /></h1>
			<g:if test="${flash.message}">
			<div class="message" role="status">${flash.message}</div>
			</g:if>
			<g:datatablehelper ctrlid="table"
				cols="['Id', 'Rol']" class="table table-striped table-bordered nuevocolortabla"></g:datatablehelper>
		</div>
		<script type="text/javascript">
